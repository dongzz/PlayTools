...
public class PlayMice {
    ...
    private static var canMove = false
    ...
    public func setupScrollWheelHandler() {
        AKInterface.shared!.setupScrollWheel({deltaX, deltaY, phase in
...
//                let eventConsumed = !mode.visible
    ...
    public func setupMouseMoveHandler() {
        AKInterface.shared!.setupMouseDwon({() in
            mode.show(false)
            PlayMice.canMove = true
            return true
        })
        AKInterface.shared!.setupMouseUp({() in
            mode.show(true)
            PlayMice.canMove = false
            return true
        })
...
class CameraAction: Action {
    ...
    init(data: MouseArea) {
        ...
        PlayMice.shared.cameraScaleHandler[PlayMice.elementName] = self.moveUpdated
    }
